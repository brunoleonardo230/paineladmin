    ...
    public function render()
    {
        ...
        foreach ($navigation as $nav) {
            ...
            if (array_key_exists('activeMenu', $nav)) {
                $active = "{$nav['activeMenu']}";
            }
            $html .= "<li class='m-menu__item m-menu__item--submenu {$active}' aria-haspopup='true' m-menu-submenu-toggle='hover'>";
            $html .= "<a href='javascript:;' class='m-menu__link m-menu__toggle'>
                        <i class='m-menu__link-icon fa {$nav['icon']}'></i>
            ...
            foreach ($nav['pages'] as $page) {
                $isActive = $page['active'] == true ? "m-menu__item--active" : '';
                $html .= "<li class='m-menu__item {$isActive}' aria-haspopup='true'>
                            <a class='m-menu__link' title='{$page['label']}' href='{$page['uri']}'>
        ...
        if (!empty($permissoes)) {
            foreach ($permissoes as $key => $permissao) {
                ...
                if ($activeController == strtolower($permissao->rcs_nome)) {
                    $navigation[$permissao->ctr_nome]['activeMenu'] = 'm-menu__item--open m-menu__item--expanded';
                }
